...
const measuredSmokeReceipt: McpSmokeReceipt = {
  ...
  startup: {
    ...
    resources: 6,
    prompts: 1,
    toolCatalogChars: 4078,
    toolCatalogBudgetChars: 4500,
    largestToolManifestChars: 765,
...
describe("teacher packet generation", () => {
  ...
  it("renders a reviewer workflow that demonstrates compact-first MCP usage", () => {
    ...
    expect(workflow).toContain("Tool catalog: 4,078 / 4,500 characters.");
  ...
  it("exposes payload stats for the visual showcase", () => {
    ...
    expect(data.mcpStats.measuredSmoke?.rows[0].value).toContain("4,078 / 4,500");
