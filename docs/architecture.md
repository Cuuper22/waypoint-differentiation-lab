...
The default MCP structured output is compact: recommendation IDs, short teacher actions, material IDs, evidence IDs, quality status, and next tool hints. The text channel is intentionally brief so clients do not pay twice for the same data; `src/mcp-budgets.ts` feeds both the stdio smoke test and the showcase budget panel for every public text-returning tool and compact summary resource. The same smoke test also caps the tool catalog itself, so startup metadata stays lean. Quote-level traces remain available through `explain_modification`, the full audit table remains available through `render_evidence_audit({ detail: "full" })`, and `generate_teacher_packet({ detail: "full" })` returns the full handout when a client actually needs presentation-ready material. Compact is a default route, not a limitation.
